...
echo Natural Object-Role Modeling Architect for Visual Studio 2008 Installation

::See if a current NORMA version is already installed
REG QUERY "HKCR\Installer\Products\BA175D4200004AF40123100000000000" 1>NUL 2>&1
...
FOR /F "usebackq skip=2 tokens=2*" %%A IN (`REG QUERY "HKCR\Installer\Products\BA175D4200004AF40123100000000000" /v "PackageCode"`) DO SET InstalledNORMAPackageCode=%%B
IF '%InstalledNORMAPackageCode%'=='BA175D4200004AF40123100001210613' (
...
start /w msiexec.exe /x {24D571AB-0000-4FA4-1032-010000000000} /qb
REG QUERY "HKCR\Installer\Products\BA175D4200004AF40123100000000000" 1>NUL 2>&1
IF NOT ERRORLEVEL 1 (
...
start msiexec.exe /i "%~dp0Natural ORM Architect for Visual Studio 2008.msi"
goto :EOF
