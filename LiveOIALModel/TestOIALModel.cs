...
namespace Neumont.Tools.ORM.TestOIALModel
{
	...
	/// <summary>
	/// Provides data for the <see cref="E:Neumont.Tools.ORM.TestOIALModel.OIALModel.OIALRegenerating" /> event.
	/// </summary>
	public class OIALRegenerationEventArgs : ModelingEventArgs
	{
		/// <summary>
		/// Specifies the <see cref="T:Neumont.Tools.ORM.TestOIALModel.OIALModel"/> that raised the event.
		/// </summary>
		private readonly LiveOIALModel myOIALModel;
		/// <summary>
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="T:Neumont.Tools.ORM.TestOIALModel.OIALModel.OIALRegenerationEventArgs" /> class.	
		/// </summary>
		public OIALRegenerationEventArgs(LiveOIALModel oialModel, Transaction transaction)
		{
			if (oialModel == null)
			{
		...
		/// <summary>
		/// Gets the <see cref="T:Neumont.Tools.ORM.TestOIALModel.OIALModel"/> that raised the event.
		/// </summary>
		public LiveOIALModel OIALModel
		{
			get
			{
	...
	public partial class LiveOIALModel
	{
		...
		public static void ValidateModel(ModelElement element, INotifyElementAdded notifyAdded)
		{
			...
			LiveOIALModel oial = OIALModelHasORMModel.GetOIALModel(model);
			if (oial == null)
			{
				oial = new LiveOIALModel(model.Store);
				oial.ORMModel = model;
				if (notifyAdded != null)
				{
		...
		private sealed class OialObjectTypeFixupListener : DeserializationFixupListener<ObjectType>
		{
			...
			protected sealed override void ProcessElement(ObjectType element, Store store, INotifyElementAdded notifyAdded)
			{
				ObjectType objectType = element as ObjectType;
				ORMModel model = objectType.Model;
				LiveOIALModel oil = OIALModelHasORMModel.GetOIALModel(model);
				if (oil == null)
				{
	...
	public partial class LiveOIALModel
	{
		...
		#region Initializers
		/// <summary>
		/// A <see cref="T:SortedDictionary{Guid, AbsorbedFactType}"/> which keeps track of the Guids of <see cref="FactType"/> objects which
		...
		/// this <see cref="T:OIALModel"/> objects.
		/// </summary>
		...
		/// should be noted by this <see cref="T:OIALModel"/>
		/// </summary>
		private void ProcessModelForTopLevelTypes()
		{
			...
			for (int i = thisConceptTypeCollection.Count - 1; i >= 0; --i)
			{
				DeleteConceptTypes(thisConceptTypeCollection[i]);
			}
		...
		private void DeleteConceptTypes(ConceptType parent)
		{
			for (int i = parent.AbsorbedConceptTypeCollection.Count - 1; i >= 0; --i)
			{
				DeleteConceptTypes(parent.AbsorbedConceptTypeCollection[i]);
			}
			parent.Delete();
		}
		/// <summary>
		...
		/// A collection of <see cref="T:Neumont.Tools.ORM.ObjectModel.FactType"/>s which represents
		...
		private void FactTypeAbsorption(LinkedElementCollection<FactType> modelFactTypes)
		{
			...
			{
				AbsorbedFactType absorbedFactType = CalculateAbsorptionForOneToOneFactType(factType);
				if (!absorbedFactType.Equals(AbsorbedFactType.Empty))
				{
					myAbsorbedFactTypes.Add(factType.Id, absorbedFactType);
				}
			}
		}
		/// <summary>
		/// Calculates the absorption for a one-to-one fact type.
		/// </summary>
		/// <param name="factType">The <see cref="T:Neumont.Tools.ORM.ObjectModel.FactType"/> to calculate absorption for.</param>
		/// <returns>A structure which determines how the fact type is absorbed.</returns>
		private AbsorbedFactType CalculateAbsorptionForOneToOneFactType(FactType factType)
		{
			AbsorbedFactType retVal = AbsorbedFactType.Empty;
...
firstObject.PreferredIdentifier.FactTypeCollection;
					...
					retVal = new AbsorbedFactType(absorberGuid, type);
				}
			}
			return retVal;
		}
		...
		private static ConceptType GetConceptType(LinkedElementCollection<ConceptType> conceptTypes, ObjectType objectType, Store store)
		{
		...
		private static InformationTypeFormat GetInformationTypeFormat(ObjectType valueType)
		{
			return InformationTypeFormatHasObjectType.GetInformationTypeFormat(valueType);
		}
		/// <summary>
		/// Gets a <see cref="System.Collections.Generic.List&lt;ConceptTypeChild&gt;"/> that contains all the
		/// <see cref="Neumont.Tools.ORM.TestOIALModel.ConceptTypeChild"/> relationships that have <see cref="Neumont.Tools.ORM.ObjectModel.Role"/>
		...
		private struct AbsorbedFactType : IEquatable<AbsorbedFactType>
		{
			...
			/// <summary>
			/// Gets an empty version of this <see cref="T:Neumont.Tools.ORM.OIALModel.OIALModel.AbsorbedFactType"/>.
			/// </summary>
			public static AbsorbedFactType Empty
			{
				get { return new AbsorbedFactType(); }
			}

			/// <summary>
			/// Compares two <see cref="T:Neumont.Tools.ORM.TestOIALModel.OIALModel.AbsorbedFactType"/> elements
			/// for equality.
			/// </summary>
			/// <param name="firstAbsorbedFactType">The first
			/// <see cref="T:Neumont.Tools.ORM.TestOIALModel.OIALModel.AbsorbedFactType"/> to compare.</param>
			/// <param name="secondAbsorbedFactType">The second
			/// <see cref="T:Neumont.Tools.ORM.TestOIALModel.OIALModel.AbsorbedFactType"/> to compare.</param>
			/// <returns><see langword="true"/> if the two
			/// <see cref="T:Neumont.Tools.ORM.TestOIALModel.OIALModel.AbsorbedFactType"/> elements are eqaul.
			...
			/// <see cref="T:Neumont.Tools.ORM.TestOIALModel.OIALModel.AbsorbedFactType"/> to compare.</param>
			/// <returns><see langword="true"/> if the two
			/// <see cref="T:Neumont.Tools.ORM.TestOIALModel.OIALModel.AbsorbedFactType"/> elements are not equal.
			/// Otherwise, <see langword="false"/>.</returns>
			public static bool operator !=(AbsorbedFactType firstAbsorbedFactType, AbsorbedFactType secondAbsorbedFactType)
			{
				return firstAbsorbedFactType.myAbsorberId != secondAbsorbedFactType.myAbsorberId ||
