...
namespace ORMSolutions.ORMArchitect.ORMToORMAbstractionBridge
{
	public partial class AbstractionModelIsForORMModel
	{
		...
		private static partial class ModificationTracker
		{
			...
			{
				if (e.DomainProperty.Id == ValueTypeHasDataType.AutoGeneratedDomainPropertyId)
				{
					SignificantObjectTypeChange(((ValueTypeHasDataType)e.ModelElement).ValueType);
				}
			}
			/// <summary>
			...
			private static void DataTypeRolePlayerChangedRule(RolePlayerChangedEventArgs e)
			{
				...
				{
					ValueTypeHasDataType link = (ValueTypeHasDataType)e.ElementLink;
					if (link.AutoGenerated && ((DataType)e.NewRolePlayer).AutoGenerationIncremental != ((DataType)e.OldRolePlayer).AutoGenerationIncremental)
					{
						SignificantObjectTypeChange(link.ValueType);
					}
				}
