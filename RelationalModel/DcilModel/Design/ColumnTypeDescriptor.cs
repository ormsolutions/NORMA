...
namespace ORMSolutions.ORMArchitect.RelationalModels.ConceptualDatabase.Design
{
		...
		where TModelElement : Column
	{
		...
		protected override ElementPropertyDescriptor CreatePropertyDescriptor(ModelElement requestor, DomainPropertyInfo domainPropertyInfo, Attribute[] attributes)
		{
			...
			else if (propertyId == Column.DataTypeAutoGeneratedDomainPropertyId)
			{
		...
		protected override bool IsPropertyDescriptorReadOnly(ElementPropertyDescriptor propertyDescriptor)
		{
			...
			Guid domainPropertyId = propertyDescriptor.DomainPropertyInfo.Id;
			if (domainPropertyId == Column.DataTypeDomainPropertyId)
			{
			...
			else if (domainPropertyId == Column.DataTypeAutoGeneratedDomainPropertyId)
			{
				if (null != (column = propertyDescriptor.ModelElement as Column) &&
					null != (valueType = column.AssociatedValueType) &&
					valueType.DataType.AutoGeneratable == AutoGenerationSupport.Required)
				{
					return true;
				}
			}
		...
		protected override bool ShouldCreatePropertyDescriptor(ModelElement requestor, DomainPropertyInfo domainProperty)
		{
			...
			{
				return null != (valueType = column.AssociatedValueType) && valueType.DataType.AutoGeneratable != AutoGenerationSupport.Never;
			}
			else if (propertyId == Column.IsIdentityDomainPropertyId)
			{
				// This is currently never set to true, do not show it. AutoGenerated is effectively conveying the same information.
				return false;
			}
		...
		#region DataTypeAutoGeneratedPropertyDescriptor class
		/// <summary>
		...
		private sealed class DataTypeAutoGeneratedPropertyDescriptor : ElementPropertyDescriptor
		{
			...
			public override bool ShouldSerializeValue(object component)
			{
				...
				if (null != (column = this.ModelElement as Column) &&
					null != (valueType = column.AssociatedValueType) &&
					null != (dataTypeUse = ValueTypeHasDataType.GetLinkToDataType(valueType)))
				{
		...
		#endregion // DataTypeAutoGeneratedPropertyDescriptor class
