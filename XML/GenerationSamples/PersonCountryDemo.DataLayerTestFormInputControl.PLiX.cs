...
namespace PersonCountryDemo
{
	...
	public partial class Create_PersonCore_InputControl : UserControl
	{
		...
		private void btnCreate_Click(object sender, System.EventArgs e)
		{
			try
			{
				this.abstractTypeVar = this.testVar.CreatePerson(int.Parse(this.dgvCreate.Rows[0].Cells["Person_id"].Value.ToString()), this.dgvCreate.Rows[0].Cells["LastName"].Value.ToString(), this.dgvCreate.Rows[0].Cells["FirstName"].Value.ToString());
				if (this.abstractTypeVar != null)
				{
	...
	public partial class Select_PersonCore_InputControl : UserControl
	{
		...
		private void btnSelect_Click(object sender, System.EventArgs e)
		{
			try
			{
				...
				{
					case "Person_id":
						this.abstractTypeVar = this.testVar.GetPersonByPerson_id(int.Parse(this.dgvSelect.Rows[0].Cells["Person_id"].Value.ToString()));
						break;
					default:
						break;
				}
