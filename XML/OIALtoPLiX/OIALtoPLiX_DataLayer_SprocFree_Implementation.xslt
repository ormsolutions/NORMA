...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="oil:model" mode="OIALtoPLiX_Implementation">
		...
		<plx:class visibility="public" modifier="sealed" name="{$ModelContextName}">
			...
			<plx:field visibility="private" name="GetConnection" dataTypeName="ConnectionDelegate"/>
			...
			<plx:function visibility="{$ModelContextInterfaceImplementationVisibility}" name="Get{$uniqueConceptTypeName}By{@name}">
				...
				<plx:try>
					...
					<plx:assign>
						...
						<plx:right>
							<plx:string>
								<plx:string>SELECT * FROM <xsl:value-of select="$Model/@name"/>.<xsl:value-of select="$uniqueConceptTypeName"/> WHERE </plx:string>
								<xsl:copy-of select="$selectString"/>
							</plx:string>
						</plx:right>
					</plx:assign>
				...
				<plx:try>
					...
					<plx:assign>
						...
						<plx:right>
							<plx:string>
								<plx:string>SELECT * FROM <xsl:value-of select="$Model/@name"/>.<xsl:value-of select="$uniqueConceptTypeName"/> WHERE </plx:string>
								<plx:string><xsl:value-of select="@name"/> = @<xsl:value-of select="@name"/>
								</plx:string>
							</plx:string>
						</plx:right>
					</plx:assign>
		...
		<plx:function visibility="{$ModelContextInterfaceImplementationVisibility}" name="Create{$ClassName}">
			...
			<xsl:variable name="columns">
				<xsl:for-each select="$mandatoryPropertiesWithPreferredIdentifiers">
					<xsl:choose>
						<xsl:when test="@parentName">
							<plx:string><xsl:value-of select="@parentName"/>_<xsl:value-of select="@name"/><xsl:if test="position() != last()">, </xsl:if></plx:string>
						</xsl:when>
						<xsl:otherwise>
							<plx:string><xsl:value-of select="@name"/><xsl:if test="position() != last()">, </xsl:if></plx:string>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:for-each>
			</xsl:variable>

			<xsl:variable name="columnParameters">
				<xsl:for-each select="$mandatoryPropertiesWithPreferredIdentifiers">
					<xsl:choose>
						<xsl:when test="@parentName">
							<plx:string>@<xsl:value-of select="@parentName"/><xsl:value-of select="@name"/><xsl:if test="position() != last()">, </xsl:if></plx:string>
						</xsl:when>
						<xsl:otherwise>
							<plx:string>@<xsl:value-of select="@name"/><xsl:if test="position() != last()">, </xsl:if></plx:string>
						</xsl:otherwise>
			...
			<plx:assign>
				...
				<plx:right>
					<plx:string>
						<plx:string>INSERT INTO <xsl:value-of select="$Model/@name"/>.<xsl:value-of select="$ClassName"/> (</plx:string>
						<xsl:copy-of select="$columns"/>
						<plx:string>) VALUES (</plx:string>
						<xsl:copy-of select="$columnParameters"/>
			...
			<plx:get>
				...
				<plx:try>
					...
					<plx:assign>
						...
						<plx:right>
							<plx:string>SELECT * FROM <xsl:value-of select="$Model/@name"/>.<xsl:value-of select="@name"/></plx:string>
						</plx:right>
					</plx:assign>
					<plx:branch>
						<plx:condition>
