...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:oldRoot="http://schemas.neumont.edu/ORM/2006-01/ORMRoot"
	xmlns:oldDiagram="http://schemas.neumont.edu/ORM/2006-01/ORMDiagram"
	...
	<xsl:import href="CoreUpgradeTo2006-04.xslt"/>
	...
	<xsl:template match="oldRoot:ORM2">
		...
		<xsl:element name="ormRoot:ORM2">
			<xsl:call-template name="AddNamespacePrefix">
				...
				<xsl:with-param name="Namespace" select="concat('http://schemas.neumont.edu/ORM/2006-04/ORMCore',@name)"/>
			</xsl:call-template>
			<xsl:call-template name="AddNamespacePrefix">
				<xsl:with-param name="Prefix" select="'ormDiagram'"/>
				<xsl:with-param name="Namespace" select="concat('http://schemas.neumont.edu/ORM/2006-04/ORMDiagram',@name)"/>
			</xsl:call-template>
			<xsl:for-each select="namespace::*[local-name()!='xml' and .!='http://schemas.neumont.edu/ORM/2006-01/ORMDiagram' and .!='http://schemas.neumont.edu/ORM/2006-01/ORMCore' and .!='http://schemas.neumont.edu/ORM/2006-01/ORMRoot']">
				<xsl:copy-of select="."/>
			</xsl:for-each>
