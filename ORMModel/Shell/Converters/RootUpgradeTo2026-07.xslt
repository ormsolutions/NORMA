...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:oldRoot="http://schemas.neumont.edu/ORM/2006-04/ORMRoot"
	xmlns:oldDiagram="http://schemas.neumont.edu/ORM/2006-04/ORMDiagram"
	xmlns:ormRoot="http://schemas.neumont.edu/ORM/2026-07/ORMRoot"
	xmlns:ormDiagram="http://schemas.neumont.edu/ORM/2026-07/ORMDiagram"
	xmlns:exsl="http://exslt.org/common"
	exclude-result-prefixes="#default xsl oldRoot oldDiagram"
	extension-element-prefixes="exsl">
	<xsl:import href="CoreUpgradeTo2026-07.xslt"/>
	...
	<xsl:template match="oldRoot:ORM2">
		...
		<ormRoot:ORM2>
			<xsl:call-template name="AddNamespacePrefix">
				...
				<xsl:with-param name="Namespace" select="concat('http://schemas.neumont.edu/ORM/2026-07/ORMCore',@name)"/>
			</xsl:call-template>
			<xsl:call-template name="AddNamespacePrefix">
				<xsl:with-param name="Prefix" select="'ormDiagram'"/>
				<xsl:with-param name="Namespace" select="concat('http://schemas.neumont.edu/ORM/2026-07/ORMDiagram',@name)"/>
			</xsl:call-template>
			<xsl:for-each select="namespace::*[local-name()!='xml' and .!='http://schemas.neumont.edu/ORM/2006-04/ORMDiagram' and .!='http://schemas.neumont.edu/ORM/2006-04/ORMCore' and .!='http://schemas.neumont.edu/ORM/2006-04/ORMRoot']">
				<xsl:copy-of select="."/>
			</xsl:for-each>
	...
	<!-- The namespace of ORM Diagram is changing because it references ORM core. This isn't worth a separate template. -->
