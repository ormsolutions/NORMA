...
namespace ORMSolutions.ORMArchitect.Core.ObjectModel
{
					...
					foreach (ORMModel model in store.ElementDirectory.FindElements<ORMModel>())
					{
						...
						for (int i = 0; i < startingCount; ++i)
						{
							...
							if (currentIndex >= 0 && currentIndex < knownTypesCount)
							{
								...
								if (existingType.AutoGeneratable == AutoGenerationSupport.Required)
								{
									// The auto generation state is not custom and is therefore not serialized on the data type uses, but
									// autogenerated should always be set in this case.
									foreach (ValueTypeHasDataType dataTypeUse in ValueTypeHasDataType.GetLinksToValueTypeCollection(existingType))
									{
										dataTypeUse.AutoGenerated = true;
									}
								}
							}
						}
	...
	public abstract partial class DataType
	{
		...
		/// do any facet checking to see if one value type can store the other,
		...
		{
			ValueTypeHasDataType dataTypeLink1 = ValueTypeHasDataType.GetLinkToDataType(valueType1);
			ValueTypeHasDataType dataTypeLink2 = ValueTypeHasDataType.GetLinkToDataType(valueType2);
			DataType dataType1 = dataTypeLink1.DataType;
			DataType dataType2 = dataTypeLink2.DataType;
			if (dataType1 == dataType2)
			{
				switch (dataType1.PortableDataType)
				{
					...
					case PortableDataType.NumericAutoCounter:
						// Only support this if the value types are the same. Otherwise, we're mapping different
						// counters to each other, which is meaningless.
						// Auto counters require auto generatable, so we can do this simple check.
						return valueType1 == valueType2;
					default:
						if (dataTypeLink1.AutoGenerated || dataTypeLink2.AutoGenerated)
						{
							return valueType1 == valueType2; // Do not compare auto-generated identifiers from different value types
						}
				...
				!dataTypeLink1.AutoGenerated &&
				!dataTypeLink2.AutoGenerated)
			{
		...
		/// <summary>
		/// Determine the level of support for auto-generating values of this type.
		/// </summary>
		...
		/// <summary>
		/// Generate a new unique value for this set
		/// </summary>
		/// <param name="existingValues">Existing instance strings. These will be invariant forms if that is supported by the data type.</param>
		/// <returns>A new string value, or null if a value could not be generated.</returns>
		public virtual string AutoGenerate(IEnumerable<string> existingValues)
		{
			// This will be generated for auto-generatable data types.
			return null;
		}
		/// <summary>
