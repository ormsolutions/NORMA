	...
	partial class RolePathOwner : IModelErrorOwner, IRolePathOwner
	{
		...
		private sealed class ReplaceDeprecatedElementsFixupListener : DeserializationFixupListener<RolePathOwner>
		{
			...
			protected sealed override void ProcessElement(RolePathOwner element, Store store, INotifyElementAdded notifyAdded)
			{
				if (!element.IsDeleted)
				{
					// Deprecated patterns 1-6 are considered too old too care about. These constructs were
					// iterations from the teething year of role paths (July 2009-June 2010) and written prior
					// to the time when the role path editor (in NORMA Pro) was released outside the immediate
					// team. Trying to do a file format upgrade (in XLST) for these patterns is prohibitive and
					// prohibitive to test since I would need to recreate NORMA versions from 2009 to produce
					// reliable input samples. In the highly unlikely case that someone produces a file that
					// still contains these patterns, we can load and save it with the tool before the format
					// change to eliminate these ancient and ephemeral constructs.
					// The 'intra fact type split' concept (pattern 7) was eliminated in (2013) so is more
					// recent and possibly has external exposure. This was also not a file format change,
					// just a blocked pattern. We'll leave the fixup code in place for this construct.
