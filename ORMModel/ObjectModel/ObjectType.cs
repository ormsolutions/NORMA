...
namespace ORMSolutions.ORMArchitect.Core.ObjectModel
{
	...
	partial class ObjectType : INamedElementDictionaryChild, INamedElementDictionaryParent, INamedElementDictionaryRemoteChild, IDefaultNamePattern, IModelErrorOwner, IHasIndirectModelErrorOwner, IModelErrorDisplayContext, IVerbalizeCustomChildren, IHierarchyContextEnabled
	{
		...
		private bool GetDataTypeAutoGeneratedValue()
		{
			ValueTypeHasDataType link;
			ObjectType identifyingObjectType;
			return null != (link = GetDataTypeLink()) ||
				(null != (identifyingObjectType = GetObjectTypeForPreferredConstraint()) &&
				null != (link = identifyingObjectType.GetDataTypeLink())) ? link.AutoGenerated : false;
		}
		private void SetDataTypeAutoGeneratedValue(bool newValue)
		{
			ValueTypeHasDataType link;
			ObjectType identifyingObjectType;
			if (!Store.InUndoRedoOrRollback &&
		...
		public ObjectType GetObjectTypeForPreferredConstraint()
		{
		...
		public ObjectType GetValueTypeForPreferredConstraint()
		{
			//If there is a preferred internal uniqueness constraint and that uniqueness constraint's role
			// player is a value type then return the value type.
		...
		{
			/// <summary>
			/// Create a new TestRemoveIsIndependentFixupListener
			/// </summary>
		...
		/// Test if an added data relationship causes a reference scheme or unspecified type error.
		/// </summary>
		private static void VerifyValueTypeHasDataTypeAddRule(ElementAddedEventArgs e)
		{
			// When driven by the UI this will always be an unspecified data type, but 
			ValueTypeHasDataType link = e.ModelElement as ValueTypeHasDataType;
			ObjectType valueType = link.ValueType;
			switch (link.DataType.AutoGeneratable)
			{
			...
			FrameworkDomainModel.DelayValidateElement(valueType, DelayValidateDataTypeNotSpecifiedError);
		...
		private static void VerifyValueTypeHasDataTypeRolePlayerChangeRule(RolePlayerChangedEventArgs e)
		{
			...
			{
				ValueTypeHasDataType link = e.ElementLink as ValueTypeHasDataType;
				AutoGenerationSupport oldAutoGen = ((DataType)e.OldRolePlayer).AutoGeneratable;
				AutoGenerationSupport newAutoGen = ((DataType)e.NewRolePlayer).AutoGeneratable;
				if (oldAutoGen != newAutoGen)
				{
					switch (newAutoGen)
					{
						case AutoGenerationSupport.Never:
							...
							break;
						case AutoGenerationSupport.Default:
							if (oldAutoGen == AutoGenerationSupport.Never)
							{
								link.AutoGenerated = true;
							}
							break;
				...
				FrameworkDomainModel.DelayValidateElement(link.ValueType, DelayValidateDataTypeNotSpecifiedError);
			}
			else
			{
	...
	partial class ValueTypeHasDataType : IElementLinkRoleHasIndirectModelErrorOwner
	{
		...
		private bool GetCustomAutoGeneratedValue()
		{
			if (AutoGenerated)
			{
				switch (DataType.AutoGeneratable)
				{
		...
		private void SetCustomAutoGeneratedValue(bool value)
		{
			if (!Store.InUndoRedoOrRollback)
			{
				if (DataType is UnspecifiedDataType)
				{
					// Unspecified data type is used as a placeholder for a forward data type reference.
					// We will only be here during direct deserialization.
					AutoGenerated = value;
				}
				else
				{
